...
export async function handler(event) {
  ...
  try {
    const [profileRes, reachRes, dayRes, followersRes, mediaRes, reachTotalRes] = await Promise.all([
      fetch(`${base}?fields=username,name,followers_count,follows_count,media_count,profile_picture_url&access_token=${token}`),
      // Serie diaria de alcance (para el gráfico): SIN total_value devuelve la serie por día
      fetch(`${base}/insights?metric=reach&period=day&since=${since}&until=${until}&access_token=${token}`),
      // Totales del período: views (ex-impressions, Meta la deprecó), visitas al perfil, clics a la web
      fetch(`${base}/insights?metric=views,profile_views,website_clicks&period=day&metric_type=total_value&since=${since}&until=${until}&access_token=${token}`),
      ...
      // Posts recientes con su imagen y engagement (carruseles: 1er hijo; video: thumbnail)
      fetch(`${base}/media?fields=caption,media_type,media_url,thumbnail_url,permalink,timestamp,like_count,comments_count,children{media_url,thumbnail_url,media_type},insights.metric(reach,saved)&limit=24&access_token=${token}`),
      // Alcance ÚNICO (de-duplicado) del período para el KPI — total_value
    ...
    const reachTotalJson = await safeJson(reachTotalRes);
    ...
    // Alcance ÚNICO del período (no la suma de días, que duplicaría personas)
    const reachTotal = sumMetric(reachTotalJson, 'reach') || reachValues.reduce((a, v) => a + (v.value || 0), 0);
    ...
    return json(200, {
      ...
      totals: {
        period,
        reach: reachTotal,
        impressions: sumMetric(dayJson, 'views'),
