...
async function fetchContacts() {
  const { data, error } = await supabase
    ...
    .select(`
      ...
      opportunities:opportunities!contact_id(id, stage, assigned_to, expected_amount, final_amount, unit:units!unit_of_interest_id(brand, year)),
      sales:sales!buyer_contact_id(id, sale_price, status, signed_at, created_at, unit:units!unit_id(brand, year))
    `)
    .is('deleted_at', null)
    .order('full_name');
  if (error) {
