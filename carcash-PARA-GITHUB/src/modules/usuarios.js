...
function render() {
  $('#view').innerHTML = `
    ...
    <div class="page-body">
      <div class="note" style="margin-bottom:16px">Acá creás los accesos y definís el rol de cada persona. El rol controla qué ve y qué puede hacer en el CRM. Los leads entrantes llegan sin asignar y el <b>supervisor</b> los reparte desde la Bandeja.</div>
...
function openEditModal(u) {
  ...
  modal.appendChild(el('div', { class: 'modal-body' },
    ...
    me ? null : el('div', { class: 'text-muted', style: { fontSize: '11px', marginTop: '4px' } }, 'Si lo desactivás, no podrá ingresar hasta reactivarlo.'),
  ));
  modal.appendChild(el('div', { class: 'modal-actions' },
    me ? null : el('button', { class: 'btn btn-danger btn-sm', onClick: () => removeUser(u, close) }, 'Desactivar'),
    el('div', { style: { flex: '1' } }),
    el('button', { class: 'btn btn-ghost', onClick: () => close() }, 'Cancelar'),
...
async function removeUser(u, closeModal) {
  const ok = await confirmDialog(`¿Desactivar a ${u.full_name}? Va a perder el acceso al CRM.`, { okText: 'Desactivar' });
  if (!ok) return;
  const { error } = await supabase.from('users_profile').update({ active: false, deleted_at: new Date().toISOString() }).eq('id', u.id);
  if (error) { toast('Error', error.message, 'error'); return; }
  toast('Usuario desactivado', u.full_name, 'warn');
  closeModal();
  await load(); renderUI();
}
